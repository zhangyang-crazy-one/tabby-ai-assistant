...
在特殊网络环境下，您可能需要通过 HTTP/HTTPS 代理服务器才能访问云端 AI API 端点。本插件支持配置代理服务器，以便在受限网络环境中正常使用。

### 支持的代理类型

| 类型 | 说明 | 示例 |
|------|------|------|
| **HTTP 代理** | HTTP 请求代理 | `http://127.0.0.1:7890` |
| **HTTPS 代理** | HTTPS 请求代理（推荐） | `http://127.0.0.1:7890` |
| **SOCKS5 代理** | SOCKS5 协议代理 | `socks5://127.0.0.1:1080` |

### 配置代理

1. 打开 Tabby 设置 → AI助手 → **代理设置** 标签
2. 启用 **启用代理** 开关
3. 填写代理地址：
   - **HTTP 代理**: HTTP 请求的代理地址
   - **HTTPS 代理**: HTTPS 请求的代理地址（大多数 API 使用 HTTPS）
4. （可选）配置 **No Proxy** 列表，指定不使用代理的地址
5. （可选）如果代理需要认证，启用 **需要认证** 并填写用户名和密码
6. 点击 **测试连接** 验证代理配置
7. 点击 **保存** 按钮保存配置

### 从环境变量导入

插件支持自动读取系统环境变量中的代理配置：

- `HTTP_PROXY` / `http_proxy` - HTTP 代理地址
- `HTTPS_PROXY` / `https_proxy` - HTTPS 代理地址
- `NO_PROXY` / `no_proxy` - 不使用代理的地址列表

点击 **从环境变量导入** 按钮可自动读取这些配置。
...
配置完成后，点击 **测试连接** 按钮验证代理是否正常工作。测试会尝试通过代理访问 AI API 端点并返回连接状态和延迟。

### 注意事项

- 代理设置将应用于所有云端 AI 提供商的 API 请求
- 本地提供商（Ollama、vLLM）默认不使用代理
- MCP HTTP 传输也支持代理配置
- 确保代理服务器稳定可用
...
- **v1.0.41**: 代理支持功能 (Issue #7)
  - **新增功能**: HTTP/HTTPS/SOCKS5 代理支持
  - **新增 UI**: 代理设置页面（支持中/英/日三语）
  - **新增功能**: 代理配置验证和连接测试
  - **新增功能**: 从环境变量自动导入代理配置
  - **新增功能**: 代理认证支持（用户名/密码）
  - **新增功能**: No Proxy 列表配置
  - **代码修改**: 所有云端 Provider 支持代理（OpenAI、Anthropic、GLM、Minimax、OpenAI-Compat）
  - **代码修改**: MCP HTTP 传输支持代理
  - **新增依赖**: http-proxy-agent、https-proxy-agent、socks-proxy-agent

- **v1.0.40**: OpenAI 兼容接口 DeepSeek 工具调用消息格式修复
  - **问题修复**: Agent 模式 Round 2 开始 400 错误
  - **错误信息**: "Messages with role 'tool' must be a response to a preceding message with 'tool_calls'"
  - **问题根因**:
    1. `assistant` 消息的 `tool_calls` 字段在 `transformMessages()` 中丢失
    2. `toolCalls` 字段名与 DeepSeek API 期望的 `tool_calls` 不匹配
  - **修复内容**:
    - `ai-assistant.service.ts`: 保存 `toolCalls` 字段到消息历史
    - `openai-compatible.service.ts`: `transformMessages()` 将 `toolCalls` 转换为 OpenAI 格式的 `tool_calls`
  - **影响范围**: 使用 DeepSeek、OpenAI 兼容站点的所有 Agent 模式对话
